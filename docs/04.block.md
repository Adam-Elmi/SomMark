...
Inline statements, at-blocks, and text are fully supported at the top level. 

Previously, these were required to be wrapped in a Block, but in SomMark v3, top-level content is permitted and will be rendered normally (e.g., contributing directly to the body content).
...
All node types (Blocks, Inlines, AtBlocks, Text, and Comments) are allowed at the top level.
...
Inlines can exist anywhere, including the top level or inside a Block:

```lua
(Hello)->(Bold)
```

Both are valid in SomMark v3.
...
Atblocks can be used at the top level or inside a Block.

---

## 10. Top-Level Text & Inlines

You can write content directly without a wrapping block:
...
#### Rules:
* **Top-Level**: All node types (Blocks, Inlines, AtBlocks, Text, and Comments) are allowed at the top level.
* **Identifiers**: Letters, numbers, underscores (`_`), dollar signs (`$`), and hyphens (`-`) are allowed in identifiers.
* **Optional Arguments**: Supports positional (`[B=1,2]`) and key-value (`[B=id:1]`).
* **Nesting**: Blocks can be nested inside other blocks.
* **Body**: Can contain text, inlines, atblocks, or other blocks. Also can be empty.
