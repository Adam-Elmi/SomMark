...
Every standard text block passes through the `text` filter before rendering. You can use tag registration options (like `escape: false`) to selectively bypass global filters:
...
// 2. Register custom raw block tag utilizing fluent tag builder
...
The `options` argument is **completely custom**. You can define any static properties during tag registration and read them inside the filter dynamically based on active block scopes.

#### Rich Examples of Custom Options

##### 1. Configurable Censorship Filter
You can use a custom tag option to trigger text censoring inside specific blocks:
```javascript
mapper.text = function (text, options) {
  if (options?.censor) {
...
// Register tag with static censor option
...
* **Scope Restriction:** The global `text()` filter only processes standard text nodes. Text inside **Inline Statements** or **At-Blocks** is processed by their respective specialized filters: `inlineText()` and `atBlockBody()`.
