			...
			if (child.type === BLOCK) {
				...
				try {
					...
					if (val !== undefined && typeof val !== "object") {
						expanded.push({ type: TEXT, text: String(val), range: child.range, depth: child.depth });
					}
				} catch (err) {
...
async function generateOutputCore(ast, i, format, mapper_file, security = {}, parentId = null, generateRuntimeOutput = false, hideRuntimeOutput = false, instance = null, idState = null, extraCtx = {}) {
	...
	if (target) {
		...
		if (isManualMode) {
			...
			try {
				for (const child of (node.body || [])) {
					if (child.type === BLOCK || child.type === TEXT) {
						...
						try {
							...
							if (val !== undefined && typeof val !== "object") {
								...
								cleanBody.push({ type: TEXT, text: resolvedText, range: child.range, depth: child.depth });
							}
						} catch (err) {
							const line = child.range?.start?.line + 1 || 1;
							transpilerError([
