import Mapping from "../mapper.js";
const html = new Mapping();
const { tag, setHeader, highlightCode } = html;
...
// Link
html.create("link", (args, children) => {
	return tag("a").attributes({ href: args[0], title: args[1] }).body(children);
});
// Image
html.create("image", (args, children) => {
	return tag("img").selfClose().attributes({ src: args[0], alt: children }).body("");
});
// Code
html.create("code", (args, children) => {
	const [content, cssTheme] = highlightCode(children, args[0].trim());
	setHeader(null, { rawData: [cssTheme] });
	return tag("pre").body(
		tag("code")
